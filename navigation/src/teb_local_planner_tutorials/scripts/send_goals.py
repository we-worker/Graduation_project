...
def send_goals_python():
    ...
    goal0.target_pose.pose.orientation.w = 0
    ...
    goal2.target_pose.pose.orientation.z = 0.7021705987793959
    goal2.target_pose.pose.orientation.w = math.sqrt(1 - goal2.target_pose.pose.orientation.z**2)
    
    goal3.target_pose.pose.position.x = 3.75
    goal3.target_pose.pose.position.y = 2
    goal3.target_pose.pose.orientation.z = 0.7021705987793959
    goal3.target_pose.pose.orientation.w = math.sqrt(1 - goal3.target_pose.pose.orientation.z**2)
    
    goal_lists=[goal0, goal1, goal2, goal3]       # 采用 python 中的列表方式，替代实现C/C++ 中的数组概念 
    goal_number = 2     # total is 4 goals
    while(goal_number):      
        if(4 - goal_number ==0):
